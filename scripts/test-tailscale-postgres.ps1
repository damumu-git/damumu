[CmdletBinding()]
param(
    [string]$PostgresHost = '100.66.109.44',
    [string]$Database = 'damumu',
...
$testOutput = Join-Path $buildRoot ('tailscale-postgres-tests-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    & $psql -h $PostgresHost -p 5432 -U $Username -d $Database -v ON_ERROR_STOP=1 -P pager=off `
        -tAc 'SELECT current_database(), current_user'
    if ($LASTEXITCODE -ne 0) { throw "Could not connect to $PostgresHost`:5432/$Database" }
        ...
        Get-ChildItem (Join-Path $repositoryRoot 'restapi/Migrations') -Filter '*.sql' |
            ...
            ForEach-Object {
                ...
                Get-Content -LiteralPath $_.FullName -Raw |
                    & $psql -h $PostgresHost -p 5432 -U $Username -d $Database `
    ...
    if (-not $healthy) { throw 'Test API did not become healthy' }

    dotnet (Join-Path $testOutput 'Muda.Api.PaginationTests.dll') `
        "$apiUrl/" (Join-Path $repositoryRoot 'restapi/appsettings.json')
    if ($LASTEXITCODE -ne 0) { throw 'Tailscale PostgreSQL integration tests failed' }
} finally {
